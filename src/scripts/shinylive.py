...
from typing import Callable, Dict, List, Literal, Set, Tuple, TypedDict, Union, cast
...
def _shinylive_base_deps(path_prefix: str = "shinylive-dist/") -> None:
    ...
    all_files: List[str] = []
    for root, dirs, files in os.walk(shinylive_dir):
        root = Path(root)
        rel_root = root.relative_to(shinylive_dir)
        if rel_root == Path("."):
            dirs.remove("scripts")
