...
test.describe("The URL can be used to load data", async () => {
  ...
  test("Examples view", async ({ page }) => {
    await page.goto(
      makeUrl({ view: "examples", data_param: app_url_encoding })
    );
  ...
  test("App view with header bar", async ({ page }) => {
...
test.describe("The header bar parameter can turn off the header in app view but not the other views", async () => {
  test("Editor view can't turn off header bar", async ({ page }) => {
    await page.goto(
      makeUrl({ view: "editor", header: false, data_param: app_url_encoding })
    );

    await expect(page.locator('.HeaderBar img[alt="Shiny"]')).toBeVisible();
  });
  test("Examples view can't turn off header bar", async ({ page }) => {
    await page.goto(
      makeUrl({ view: "examples", header: false, data_param: app_url_encoding })
    );
  ...
  test("App view can showheader bar", async ({ page }) => {
    await page.goto(
      makeUrl({ view: "app", header: true, data_param: app_url_encoding })
    );
